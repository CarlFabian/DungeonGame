...
// Deklaration & Definition av Objekt från klasser.
   ...
   // Skapande av rum,xPos och yPos är positionen i 2d-arrayen dungeon.
...
// Combat-Metod för vanliga monster i room 4, där den skapar & initaliserar objektet "enemy" från Constructor i Monster-klassen.
   public static boolean startFight(Monster enemy, boolean playerHasPotion)
   {
      ...
      // Kallar efter monsterRandomizer metod för att bestämma ett slumpmässigt namn
      // för MonsterName String.
      ...
      while (enemy.getMonsterHp() > 0 && Player.getHealthPoints() > 0)
      {
         ...
         // switch statement som använder använder spelarens input (String val) som
         // cases.
         switch (val)
         {
         // case "f" är det case där man utför "combat".
         case "f":
            // definition av integers som hämtar slumpmässigt värde mellan 1-100, denna
            // används för en mekanik där både spelaren och monstret har en chans att missa
            // sin attack.
            Random rand = new Random();
            int randInt = rand.nextInt(100) + 1;
            int monsterInt = rand.nextInt(100) + 1;
            // För alla värden som är under eller lika med 80 med vår 1-100 random integer
            // för spelaren (randInt), så kommer spelaren att träffa sin attack.
            if (randInt <= 80)
            {
               ...
               // För alla värden över 80 så missar spelaren sin attack, och gör därav ingen
               // damage då vår monsterTakingDamage-metod inte kallas.
            } else if (randInt > 80)
            {
               System.out.println("You missed your attack, and dealt no damage!");
            }
            // För alla värden som är under eller lika med 70 med vår 1-100 random integer
            // för Monstret (monsterInt), så kommer Monstret att träffa sin attack.
            if (monsterInt <= 70)
            {
               // Kallar efter vår setter för spelarens HP, där den bestäms med vår getter för
               // player HP minus getter för monster Damage.
            ...
            // Om random integer för monstret är över 70, så missar monstret sin attack och
            // ingen subtraktion utförs på spelarens HP.
            ...
            System.out.println("\n*********************************************************************************");
            break;
         // Case "h" så används en potion och spelarens healthpoints återställs till 100.
         case "h":
            if (playerHasPotion && !playerUsedPotion)
            {
      ...
      // Om monstrets HP är mindre än 0 och spelarens HP är högre än 0 så avslutas vår
      // while-loop för combat, och text printas med att du vann över monstret.
      if (0 >= enemy.getMonsterHp() && 0 <= Player.getHealthPoints())
      {
         System.out.printf("You defeated the %s, after a glorious battle!\n", enemy.getMonsterName());

      }
      // Om while-loop avslutades där istället monstrets HP var över 0 men spelarens
      // HP var mindre eller lika med 0, så printas text att du förlorade och spelet
      // avslutas.
      else
      {
      ...
      // En boolean som säger om spelaren har använt en potion under metoden
      // returneras.
...
// Combat metod för Draken, där den användaren objektet från constructor vid room 8 i dess parameter.
   public static boolean dragonFight(Monster dragon, boolean playerHasPotion)
   {
      ...
      while (dragon.getMonsterHp() > 0 && Player.getHealthPoints() > 0)
      {
         ...
         System.out.printf("\nPress F: to fight for your life against the %s or H to use healthpotion!",
         ...
         {
         case "f":
            // Definition av random integers för drake (monsterInt) och spelare (randInt)
            // som används för vår mekanik för slumpmässig miss vid attack. Värden för denna
            // random är mellan 1-100.
            Random rand = new Random();
            int randInt = rand.nextInt(100) + 1;
            int monsterInt = rand.nextInt(100) + 1;
            // om spelarens random integer (randInt) är under eller lika med 80, så träffar
            // spelaren sin attack.
            if (randInt <= 80)
            {
               // Kallar efter metod där monstret tar den skada som den befintliga playerDamage
               // innehar(int)
               ...
               // om spelarens random integer i stället är över 80, så missar spelaren sin
               // attack och ingen skada utförs.
            } else if (randInt > 80)
            {
               System.out.println("You missed your attack and dealt no damage!");
            }
            // Om drakens random int (monsterInt) är över eller lika med 70, så gör draken
            // skada på spelaren.
            if (monsterInt <= 70)
            {
               // Kallar efter vår setter för spelarens HP, där den bestäms med vår getter för
            ...
            // Om monster random int i stället är över 70, så gör draken ingen skada på
            // spelaren.
            else if (monsterInt > 70)
            {
      ...
      // Om drakens hp är mindre än 0, och spelarens hp är över 0 så printas
      // meddelandet att spelaren vann striden.
      if (0 >= dragon.getMonsterHp() && 0 <= Player.getHealthPoints())
      {
         System.out.printf("You defeated the %s, after a glorious battle!\n", dragon.getMonsterName());

      }
      // om while-loopen istället avslutas där spelaren har 0 hp och draken har över 0
      // hp, så printas meddelandet att spelaren förlorade och spelet avslutas.
      else
      {
         System.out.println("You died! Game over!");
         DungeonMaster.endGame(false);
